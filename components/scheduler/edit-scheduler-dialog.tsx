...
export function EditSchedulerDialog({
  ...
  const [tab, setTab] = useState<"Expense" | "Income" | "Transfer" | "Invest">(
    scheduler.type as "Expense" | "Income" | "Transfer" | "Invest",
  );
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      name: scheduler.name === "Transfer" || scheduler.name === "Invest" ? "" : scheduler.name || "",
      amount: scheduler.amount || 0,
      budget_id: scheduler.budget_id || undefined,
      wallet_id: scheduler.wallet_id || undefined,
      transfer_id: scheduler.transfer_id || undefined,
      portfolio_id: scheduler.portfolio_id || undefined,
      admin_fee: scheduler.admin_fee || 0,
      frequency: scheduler.frequency as "Daily" | "Weekly" | "Monthly" | "Yearly",
  ...
  return (
    <Drawer open={open} onOpenChange={handleOpenChange} direction={isMobile ? "bottom" : "right"}>
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>Edit Scheduler</DrawerTitle>
        </DrawerHeader>
