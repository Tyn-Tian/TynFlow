...
function ChartTooltipContent({
  ...
  const items = React.useMemo(() => {
    ...
    const order = ["present", "capital"]
    arr.sort((a, b) => {
      const ak = String(a?.dataKey || a?.name || "")
      const bk = String(b?.dataKey || b?.name || "")
                      ...
                      {item.value !== undefined && (
                        <span className="text-foreground font-mono font-medium tabular-nums ml-3">
                          {(() => {
                            ...
                            // If the payload has a label that looks like our portfolio labels,
                            // format as Indonesian Rupiah. Otherwise fall back to default locale string.
                            const label = itemConfig?.label
                            const isPortfolioCurrency =
                              typeof label === "string" &&
                              (label.toLowerCase().includes("investment") ||
                                label.toLowerCase().includes("present"))

                            if (typeof value === "number") {
                              return isPortfolioCurrency
